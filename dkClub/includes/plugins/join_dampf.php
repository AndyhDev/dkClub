...
function plugin_join_dampf(){
    ...
    foreach($erg as $key => $value){
        if(!$value['ok']){
            $ok = False;
            $name = $value['field'];
            $html = $html . "Das Feld <span style='font-weight:bold;'>'$name'</span> hat foldgende Fehler:<ul>";
    ...
    if(!$ok){
        $html = $html . "<span style='font-weight:bold;'><a href='index.php?site=join_dampf'>Bitte gehen sie zurück zum Formular und korrigieren sie die Fehler</a></span>";
        return $html;
    }else{
        $sql = "INSERT INTO dampffest (Name, Vorname, Strasse, Land, PLZ, Ort, email, Tel, Samstag, Sonntag, Montag, Anzahl_Personen, Sa_Essen, _5_Zoll,
                                       ...
                                       Montag__Vormittag_Personal, Montag_Nachmittag_Personal, Dienstag_Abbau)
                                       
                VALUES ('$name', '$vorname', '$strasse', '$land', '$plz', '$ort', '$email', '$tel', '$sa', '$so', '', '$anz_p', '$sa_essen', '$zoll5',
                        '$zoll7', '$zoll10', '$lok_m', '$zug_m', '$str_dampf', '$stat_modelle', '$anz_tisch', '$strom', '$luft',
                        '$modell_z1', '$modell_z2', '$zelt', '$womo', '$auto', '$anreise', '$na1', '$na2', NULL,
                        '', '', '', '', '',
                        '', '', '')";
        ...
        Ihre Daten wurden ordnungsgemäß übermittelt (siehe unten).<br/>
        Ihre Teilnahmeunterlagen erhalten sie ca. 1 bis 2 Wochen vor dem Veranstaltungstermin. Wir freuen uns sie bald persönlich bei uns begrüßen zu dürfen.<br/>
        Sollten sie eine Unterkunftsmöglichkeit suchen so bietet das 3 km vom Museum entfernte Bad Schussenried zahlreiche Beherbergungsbetriebe an (Hotels, Pensionen usw. Zu finden unter <a href='http://bad-schussenried.de'>www.bad-schussenried.de</a><br/>
        Mit freundlichen Grüßen<br/>
        Schwäbischer Eisenbahnverein e.V.<br/><br/>
